...
- Migrations live in `supabase/migrations/` (`000_â€¦` onward).
...
- Important recent migrations:
  - `007_password_reset_and_audit.sql` â€?password reset tokens, `audit_log`
  - `008_public_registration.sql` â€?nullable `user_id`, view/signup tokens
  - `009_unique_registration_email.sql` â€?unique `lower(email)` for non-empty emails
